...
fn main_smarthouse_test() {
    ...
    let bedroom_device: Vec<Arc<dyn DeviceInterface>> = vec![
        Arc::new(Socket::new(Socket {
        ...
        Arc::new(Thermometer::new(Thermometer {
            name: thermo_bedroom_name.clone(),
            temperature: 22,
        })),
    ];
    let kitche_device: Vec<Arc<dyn DeviceInterface>> = vec![
        Arc::new(Socket::new(Socket {
    ...
    storage.room_map.insert(kitchen_name.clone(), kitche_device);
    ...
    assert!(smarthouse
        .get_rooms_devices_list(&bedroom_name)
        .contains(&socket_bedroom_name));
    assert!(smarthouse
        .get_rooms_devices_list(&bedroom_name)
        .contains(&socket_bedroom_bathroom_name));
    assert!(smarthouse
        .get_rooms_devices_list(&bedroom_name)
        .contains(&thermo_bedroom_name));

    // Проверяем список устройств на кухне
    assert!(smarthouse
        .get_rooms_devices_list(&kitchen_name)
        .contains(&socket_kitchen_name));
    assert!(smarthouse
        .get_rooms_devices_list(&kitchen_name)
        .contains(&socket_kitchen_stove_name));

    // Проверяем отчеты по устройствам
    let exp_msg = format!("'{}' не найдено в {}", socket_bedroom_name, bedroom_name);
    assert!(smarthouse
        .get_device_info(&bedroom_name, &socket_bedroom_name, &storage)
        .expect(&exp_msg)
        .contains(&socket_bedroom_name));
    assert!(smarthouse
        .get_device_info(&bedroom_name, &socket_bedroom_name, &storage)
    ...
    assert!(smarthouse
        ...
        .contains("ватт"));
    assert!(smarthouse
        .get_device_info(&bedroom_name, &socket_bedroom_bathroom_name, &storage)
        .expect(&exp_msg)
        .contains(&socket_bedroom_bathroom_name));
    assert!(smarthouse
        .get_device_info(&bedroom_name, &socket_bedroom_bathroom_name, &storage)
    ...
    assert!(smarthouse
        ...
        .contains("ватт"));
    assert!(smarthouse
        .get_device_info(&bedroom_name, &thermo_bedroom_name, &storage)
        .expect(&exp_msg)
        .contains(&thermo_bedroom_name));
    assert!(smarthouse
        .get_device_info(&bedroom_name, &thermo_bedroom_name, &storage)
    ...
    assert!(smarthouse
        .get_device_info(&bedroom_name, &thermo_bedroom_name, &storage)
        .expect(&exp_msg)
        .contains("градусов по цельсию"));
    assert!(
        match smarthouse.get_device_info(&kitchen_name, "Неведомая хрень!".as_ref(), &storage)
        {
            Ok(_) => false,
            Err(DeviceStorageGetInfoErrors::NoDevice(error_text)) =>
                error_text.device_name.contains("Неведомая хрень!"),
            Err(DeviceStorageGetInfoErrors::NoRoom(_)) => false,
        }
    )
}
